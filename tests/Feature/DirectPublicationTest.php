...
use App\Models\Role;
use App\Models\User;
use App\Services\PublishedArticleFileResolver;
...
class DirectPublicationTest extends TestCase
{
    ...
    public function test_readiness_allows_optional_metadata_and_uses_only_explicit_public_files(): void
    {
        ...
        $privateFigure = ArticleFile::create(['article_id' => $article->id, 'article_version_id' => $article->current_version_id, 'uploaded_by' => $this->superAdmin->id,
            'file_type' => ArticleFile::DIRECT_PUBLICATION_FIGURE, 'visibility' => 'internal', 'disk' => 'local', 'file_path' => 'direct/private-figure.png', 'storage_key' => 'direct/private-figure.png',
            'original_name' => 'private-figure.png', 'safe_original_name' => 'private-figure.png', 'mime_type' => 'image/png', 'size' => 100, 'scan_status' => 'clean']);
        $supplementary = ArticleFile::create(['article_id' => $article->id, 'article_version_id' => $article->current_version_id, 'uploaded_by' => $this->superAdmin->id,
            'file_type' => ArticleFile::DIRECT_PUBLICATION_SUPPLEMENTARY, 'visibility' => 'internal', 'disk' => 'local', 'file_path' => 'direct/table-s1.csv', 'storage_key' => 'direct/table-s1.csv',
            'original_name' => 'table-s1.csv', 'safe_original_name' => 'table-s1.csv', 'mime_type' => 'text/csv', 'size' => 100, 'scan_status' => 'clean']);
        ...
        $this->withHeader('Idempotency-Key', 'direct-file-visibility')->postJson("/api/admin/direct-publications/{$article->id}/public-assets", [
            'publication_file_settings' => [
                ...
                ['file_id' => $privateFigure->id, 'show_on_article' => false, 'show_in_downloads' => false, 'include_in_package' => true],
                ['file_id' => $supplementary->id, 'show_in_downloads' => true, 'include_in_package' => true],
            ],
        ])->assertOk();
        ...
        $this->assertTrue(app(ArticleFileController::class)->canAccess(null, $supplementary->fresh('article')));
        $this->assertFalse(app(ArticleFileController::class)->canAccess(null, $privateFigure->fresh('article')));
        $this->assertFalse(app(ArticleFileController::class)->canAccess(null, $source->fresh('article')));
        $publicSections = collect(app(PublishedArticleFileResolver::class)->resolvePublishedFilesPayload($article->fresh()));
        $this->assertSame([$figure->id], collect($publicSections->firstWhere('key', PublishedArticleFileResolver::SECTION_FIGURES)['files'])->pluck('id')->all());
        $this->assertSame([$supplementary->id], collect($publicSections->firstWhere('key', PublishedArticleFileResolver::SECTION_SUPPLEMENTARY_DOWNLOADS)['files'])->pluck('id')->all());
    ...
    public function test_figures_and_supplementary_files_are_independent_collections(): void
    {
        Storage::fake('s3');
        config(['media_uploads.disk' => 's3']);
        Sanctum::actingAs($this->superAdmin);
        $articleId = $this->withHeader('Idempotency-Key', 'collection-create')->postJson('/api/admin/direct-publications', $this->payload($this->magazine))->assertCreated()->json('data.id');
        $article = Article::findOrFail($articleId);

        $figures = collect([
            $this->cleanUpload($article, 'direct_publication_figure', 'figure.png', 'image/png', "\x89PNG\r\n\x1a\nfigure-one"),
            $this->cleanUpload($article, 'direct_publication_figure', 'figure.png', 'image/png', "\x89PNG\r\n\x1a\nfigure-two"),
            $this->cleanUpload($article, 'direct_publication_figure', 'figure-3.tiff', 'image/tiff', "II\x2A\x00figure-three"),
        ]);
        $supplementary = collect([
            $this->cleanUpload($article, 'direct_publication_supplementary', 'table-s1.csv', 'text/csv', "a,b\n1,2"),
            $this->cleanUpload($article, 'direct_publication_supplementary', 'methods.pdf', 'application/pdf', '%PDF-1.4 methods'),
            $this->cleanUpload($article, 'direct_publication_supplementary', 'dataset.csv', 'text/csv', "x,y\n3,4"),
        ]);

        $figureFileIds = $figures->map(fn (MediaUploadSession $upload, int $index) => $this->attachUpload($article, $upload, 'direct_publication_figure', "figure-{$index}"));
        $supplementaryFileIds = $supplementary->map(fn (MediaUploadSession $upload, int $index) => $this->attachUpload($article, $upload, 'direct_publication_supplementary', "supplementary-{$index}"));
        $this->assertSame(3, ArticleFile::where('article_id', $articleId)->where('file_type', ArticleFile::DIRECT_PUBLICATION_FIGURE)->count());
        $this->assertSame(3, ArticleFile::where('article_id', $articleId)->where('file_type', ArticleFile::DIRECT_PUBLICATION_SUPPLEMENTARY)->count());
        $this->assertCount(2, ArticleFile::whereIn('id', $figureFileIds)->where('original_name', 'figure.png')->get());

        $retryId = $this->attachUpload($article, $figures->first(), 'direct_publication_figure', 'figure-retry');
        $this->assertSame($figureFileIds->first(), $retryId);
        $this->assertSame(3, ArticleFile::where('article_id', $articleId)->where('file_type', ArticleFile::DIRECT_PUBLICATION_FIGURE)->count());

        $this->withHeader('Idempotency-Key', 'remove-middle-figure')->deleteJson("/api/admin/direct-publications/{$articleId}/files/{$figureFileIds[1]}")->assertOk();
        $this->withHeader('Idempotency-Key', 'remove-middle-supplement')->deleteJson("/api/admin/direct-publications/{$articleId}/files/{$supplementaryFileIds[1]}")->assertOk();
        $this->assertEqualsCanonicalizing([$figureFileIds[0], $figureFileIds[2]], ArticleFile::where('article_id', $articleId)->where('file_type', ArticleFile::DIRECT_PUBLICATION_FIGURE)->pluck('id')->all());
        $this->assertEqualsCanonicalizing([$supplementaryFileIds[0], $supplementaryFileIds[2]], ArticleFile::where('article_id', $articleId)->where('file_type', ArticleFile::DIRECT_PUBLICATION_SUPPLEMENTARY)->pluck('id')->all());
        $this->assertSame(1, Article::where('submission_mode', 'direct_publication')->count());

        $this->withHeader('Idempotency-Key', 'item-public-selection')->postJson("/api/admin/direct-publications/{$articleId}/public-assets", [
            'publication_file_settings' => [
                ['file_id' => $figureFileIds[0], 'show_on_article' => true],
                ['file_id' => $figureFileIds[2], 'show_on_article' => false],
                ['file_id' => $supplementaryFileIds[0], 'show_in_downloads' => true],
                ['file_id' => $supplementaryFileIds[2], 'show_in_downloads' => false],
            ],
        ])->assertOk();
        $record = PublicationRecord::where('article_id', $articleId)->firstOrFail();
        $this->assertDatabaseHas('publication_file_selections', ['publication_record_id' => $record->id, 'article_file_id' => $figureFileIds[0], 'is_public' => true]);
        $this->assertDatabaseMissing('publication_file_selections', ['publication_record_id' => $record->id, 'article_file_id' => $figureFileIds[2]]);
        $this->assertSame(PublishedArticleFileResolver::SECTION_FIGURES, app(PublishedArticleFileResolver::class)->resolveSectionKey(ArticleFile::findOrFail($figureFileIds[0]), $article));
        $this->assertSame(PublishedArticleFileResolver::SECTION_SUPPLEMENTARY_DOWNLOADS, app(PublishedArticleFileResolver::class)->resolveSectionKey(ArticleFile::findOrFail($supplementaryFileIds[0]), $article));
    }

    public function test_file_failure_is_isolated_and_readiness_reports_processing_and_failed_sessions(): void
    {
        Storage::fake('s3');
        config(['media_uploads.disk' => 's3']);
        Sanctum::actingAs($this->superAdmin);
        $articleId = $this->withHeader('Idempotency-Key', 'failure-create')->postJson('/api/admin/direct-publications', $this->payload($this->magazine))->assertCreated()->json('data.id');
        $article = Article::findOrFail($articleId);
        $good = $this->cleanUpload($article, 'direct_publication_figure', 'good.png', 'image/png', "\x89PNG\r\n\x1a\ngood");
        $goodFileId = $this->attachUpload($article, $good, 'direct_publication_figure', 'good-figure');
        $failed = $this->uploadSession($article, 'direct_publication_supplementary', 'bad.exe', 'application/octet-stream', MediaUploadSession::STATUS_SCAN_FAILED);
        $processing = $this->uploadSession($article, 'direct_publication_figure', 'processing.png', 'image/png', MediaUploadSession::STATUS_SCANNING);

        $this->withHeader('Idempotency-Key', 'failed-attach')->postJson("/api/admin/direct-publications/{$articleId}/files", [
            'upload_id' => $failed->id, 'purpose' => 'direct_publication_supplementary',
        ])->assertUnprocessable();
        $this->assertDatabaseHas('article_files', ['id' => $goodFileId, 'article_id' => $articleId]);
        $this->getJson("/api/admin/direct-publications/{$articleId}/readiness")->assertUnprocessable()
            ->assertJsonPath('errors.publication_files_processing.0', '1 publication file is still processing.')
            ->assertJsonPath('errors.publication_files_failed.0', '1 publication file failed validation.');
        $processing->update(['status' => MediaUploadSession::STATUS_ABORTED]);
        $failed->update(['status' => MediaUploadSession::STATUS_ABORTED]);
        $readiness = $this->getJson("/api/admin/direct-publications/{$articleId}/readiness")->assertUnprocessable();
        $this->assertArrayNotHasKey('publication_files_processing', $readiness->json('errors'));
        $this->assertArrayNotHasKey('publication_files_failed', $readiness->json('errors'));
    }

    public function test_final_pdf_and_cover_are_single_active_files_with_preserved_history(): void
    {
        Storage::fake('s3');
        config(['media_uploads.disk' => 's3']);
        Sanctum::actingAs($this->superAdmin);
        $articleId = $this->withHeader('Idempotency-Key', 'singleton-create')->postJson('/api/admin/direct-publications', $this->payload($this->magazine))->assertCreated()->json('data.id');
        $article = Article::findOrFail($articleId);
        $pdfOneUpload = $this->cleanUpload($article, 'direct_publication_manuscript', 'first.pdf', 'application/pdf', '%PDF-1.4 first');
        $pdfOne = $this->attachUpload($article, $pdfOneUpload, 'direct_publication_manuscript', 'pdf-one');
        $pdfTwo = $this->attachUpload($article, $this->cleanUpload($article, 'direct_publication_manuscript', 'second.pdf', 'application/pdf', '%PDF-1.4 second'), 'direct_publication_manuscript', 'pdf-two');
        $this->assertSame($pdfOne, $this->attachUpload($article, $pdfOneUpload, 'direct_publication_manuscript', 'pdf-one-late-retry'));
        $coverOne = $this->attachUpload($article, $this->cleanUpload($article, 'direct_publication_cover', 'cover-one.png', 'image/png', "\x89PNG\r\n\x1a\ncover-one"), 'direct_publication_cover', 'cover-one');
        $this->withHeader('Idempotency-Key', 'select-first-cover')->postJson("/api/admin/direct-publications/{$articleId}/public-assets", [
            'publication_file_settings' => [['file_id' => $coverOne, 'show_on_article' => true]],
        ])->assertOk();
        $coverTwo = $this->attachUpload($article, $this->cleanUpload($article, 'direct_publication_cover', 'cover-two.png', 'image/png', "\x89PNG\r\n\x1a\ncover-two"), 'direct_publication_cover', 'cover-two');

        $this->assertSame(4, ArticleFile::where('article_id', $articleId)->count());
        $this->assertFalse((bool) data_get(ArticleFile::findOrFail($pdfOne)->metadata, 'direct_publication.active'));
        $this->assertTrue((bool) data_get(ArticleFile::findOrFail($pdfTwo)->metadata, 'direct_publication.active'));
        $this->assertFalse((bool) data_get(ArticleFile::findOrFail($coverOne)->metadata, 'direct_publication.active'));
        $this->assertTrue((bool) data_get(ArticleFile::findOrFail($coverTwo)->metadata, 'direct_publication.active'));
        $this->assertSame($pdfTwo, PublicationRecord::where('article_id', $articleId)->value('primary_publication_file_id'));
        $this->assertDatabaseMissing('publication_file_selections', ['article_file_id' => $coverOne]);
        $this->withHeader('Idempotency-Key', 'reselect-old-cover')->postJson("/api/admin/direct-publications/{$articleId}/public-assets", [
            'publication_file_settings' => [['file_id' => $coverOne, 'show_on_article' => true]],
        ])->assertUnprocessable();
        $this->getJson("/api/admin/direct-publications/{$articleId}")->assertOk()
            ->assertJsonPath('data.latest_publication_record.primary_publication_file_id', $pdfTwo);
    }
    ...
    private function cleanUpload(Article $article, string $purpose, string $filename, string $mime, string $contents): MediaUploadSession
    {
        $session = $this->uploadSession($article, $purpose, $filename, $mime, MediaUploadSession::STATUS_CLEAN, $contents);
        Storage::disk('s3')->put($session->s3_clean_key, $contents);

        return $session;
    }

    private function uploadSession(Article $article, string $purpose, string $filename, string $mime, string $status, string $contents = 'pending'): MediaUploadSession
    {
        $id = (string) Str::uuid();

        return MediaUploadSession::create([
            'id' => $id, 'user_id' => $this->superAdmin->id, 'client_upload_id' => (string) Str::uuid(),
            'purpose' => $purpose, 'attachable_type' => Article::class, 'attachable_id' => $article->id,
            'original_filename' => $filename, 'safe_display_filename' => $filename, 'expected_size_bytes' => strlen($contents),
            'declared_mime_type' => $mime, 'detected_mime_type' => $mime, 'disk' => 's3',
            's3_incoming_key' => "incoming/{$id}/{$filename}", 's3_clean_key' => $status === MediaUploadSession::STATUS_CLEAN ? "clean/{$id}/{$filename}" : null,
            'upload_mode' => 'single', 'status' => $status, 'expires_at' => now()->addHour(), 'scanned_at' => $status === MediaUploadSession::STATUS_CLEAN ? now() : null,
            'failure_reason' => $status === MediaUploadSession::STATUS_SCAN_FAILED ? 'invalid_file' : null,
        ]);
    }

    private function attachUpload(Article $article, MediaUploadSession $upload, string $purpose, string $key): int
    {
        return (int) $this->withHeader('Idempotency-Key', $key)->postJson("/api/admin/direct-publications/{$article->id}/files", [
            'upload_id' => $upload->id, 'purpose' => $purpose,
