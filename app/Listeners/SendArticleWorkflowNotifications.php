...
class SendArticleWorkflowNotifications
{
        ...
        foreach ($recipients as $recipient) {
            if (($recipient['user_id'] ?? null)
                && ! ($this->preferences ?? app(NotificationPreferenceService::class))
                    ->shouldSendLegacyEmail(User::findOrFail($recipient['user_id']), $event->event)) {
                continue;
            }
            ...
            $queued = $this->notificationService->send(
                ...
                $message['body'],
                array_key_exists('action', $message) ? $message['action'] : $this->actionForRecipient($article, $recipient),
                'default',
    ...
    private function messageForRecipient($article, ArticleWorkflowEventOccurred $event, array $recipient): array
    {
        ...
        if ($recipient['type'] === 'co_author') {
            $message['action'] = null;
            if ($event->event === 'revision.requested') {
                $message['body'] = [
                    'The editorial team requested revisions for the manuscript “'.e($article->title).'”.',
                    'Next Action: The submitting or corresponding author is responsible for coordinating and submitting the revision.',
                ];
            } elseif ($event->event === 'author.final_review_requested') {
                $message['body'] = [
                    'The manuscript “'.e($article->title).'” has reached final publication review.',
                    'Next Action: The submitting or corresponding author must complete the final approval. No action is required from other co-authors.',
                ];
            }
        }
    ...
    private function subEditorAssignmentMessage($article, ArticleWorkflowEventOccurred $event, array $recipient): array
    {
        ...
        if (in_array($recipient['type'], ['article_owner', 'corresponding_author', 'co_author'], true)) {
            return [
    ...
    private function productionAssignmentMessage($article, ArticleWorkflowEventOccurred $event, array $recipient): array
    {
        ...
        $authorRecipient = in_array($recipient['type'], ['article_owner', 'corresponding_author', 'co_author'], true);
    ...
    {
        $isAuthor = in_array($recipient['type'], ['article_owner', 'corresponding_author', 'co_author'], true);
        if ($isAuthor) {
    ...
    private function nextRevisionTrackingCode($article): string
    {
        return ($article->tracking_code ?? 'Not assigned').'-R'.max(1, (int) $article->versions()->max('version_number'));
    }

    private function subEditorRecipients($article): Collection
    {
        return collect($article->subEditorAssignments()->with('subEditor')->get()->map(fn ($a) => $this->userRecipient($a->subEditor, 'sub_editor'))->all());
    }
        ...
        return collect([
            ...
            $article->articleAuthors
                ->map(fn ($author) => [
                    ...
                    'type' => ($author->is_owner || $author->is_corresponding) ? 'corresponding_author' : 'co_author',
                ])
        )->pipe(fn ($items) => $this->dedupe($items));
    }
    ...
    {
        return match ($type) {
            'article_owner', 'corresponding_author', 'co_author' => 'Author',
            'reviewer' => 'Reviewer',
