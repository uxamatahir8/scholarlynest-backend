...
use App\Models\ArticleShareClick;
...
use App\Services\ArticleSubmissionService;
...
class ArticleController extends Controller
{
    ...
    protected ArticleSubmissionService $submissionService;

    protected AcceptedFileSetService $acceptedFileSetService;

    protected MediaStorageService $mediaStorage;

    public function __construct(PdfGeneratorService $pdfService, NotificationService $notificationService, ArticleVersionService $versionService, MediaStorageService $mediaStorage, AcceptedFileSetService $acceptedFileSetService, ArticleSubmissionService $submissionService)
    {
    ...
    public function trackShareClick(Request $request, int $id): JsonResponse
    {
        ...
        $shareClick = ArticleShareClick::firstOrCreate(
            [
    ...
    public function store(StoreArticleRequest $request): JsonResponse
    {
        ...
        $article = $this->submissionService->submit(function () use ($articleData, $request, $authors, $user, $requestedStatus, $validated) {
            ...
            $articleData['user_id'] = $articleOwner->id;
            ...
            foreach ($validated['additional_manuscript_files'] ?? [] as $additional) {
                if (empty($additional['upload_id'])) {
                    throw ValidationException::withMessages([
                        'additional_manuscript_files' => ['Each additional manuscript file must use an upload owned by the submitting author.'],
                    ]);
                }
                $upload = app(CleanUploadResolver::class)->resolveOwned($user, $additional['upload_id'], 'additional_manuscript_file');
    ...
    public function adminList(Request $request): JsonResponse
    {
        ...
        $query = $this->scopedAdminArticleQuery($user, $scopeUser, $observedUser)
            ...
            ->with([
                ...
                'currentVersion.editorialDecisions', 'activeAcceptedFileSet', 'productionAssignments', 'proofRounds',
    ...
    /**
     * Distinct filter values available inside the caller's scoped article registry.
     */
    public function adminFilterOptions(Request $request): JsonResponse
    {
        $user = $request->user();
        if (! $user) {
            return response()->json(['message' => 'Unauthenticated.'], 401);
        ...
        if ($user->hasRole('author') && ! $user->hasPermission('articles.view-any')) {
        ...
        $authors = $query
            ...
            ->with('user:id,name')
            ->get()
            ->map(fn ($article) => $article->user ? ['id' => $article->user->id, 'name' => $article->user->name] : null)
            ->filter()
    ...
    public function update(UpdateArticleRequest $request, int $id): JsonResponse
    {
        ...
        if (in_array(ArticleStatus::normalize($status), [ArticleStatus::SUBMITTED, ArticleStatus::RESUBMITTED], true)) {
            $draftManuscripts = ArticleFile::query()
                ...
                ->where('scan_status', 'clean')
                ->get();
            if ($draftManuscripts->isEmpty()) {
                return response()->json(['message' => PrimaryManuscriptService::MISSING_MESSAGE], 422);
            }
            if ($draftManuscripts->count() !== 1) {
                return response()->json(['message' => PrimaryManuscriptService::INVALID_MESSAGE], 422);
            }
        ...
        \DB::transaction(function () use ($article, $updateData, $request, $authorResolution, $articleOwner, $user, $status, $oldStatus, $linkedFileIds, &$transitionApplied) {
            ...
            if (ArticleStatus::normalize($status) !== ArticleStatus::normalize($oldStatus)) {
                if (ArticleStatus::normalize($status) === ArticleStatus::SUBMITTED && ArticleStatus::normalize($oldStatus) === ArticleStatus::DRAFT) {
                    event(new ArticleSubmitted(
                        $lockedArticle->fresh(['articleAuthors', 'magazine', 'user']),
                        $this->notificationAuthors($authorResolution['authors'], $articleOwner->email)
                    ));
                }
                $this->dispatchStatusWorkflowEvent($lockedArticle->fresh(), $user, $oldStatus, $status);
            }
        });
    ...
    public function adminStats(Request $request): JsonResponse
    {
        ...
        $magazinesQuery = Magazine::query();
        if ($magazineIds !== null) {
            $magazinesQuery->whereIn('id', $magazineIds);
        }
        $totalMagazines = $magazinesQuery->count();
        $totalUsers = User::count(); // general users count remains system-wide
    ...
    private function publicArticlePayload(Article $article, bool $includeBody = false): array
    {
        ...
        $payload = [
            ...
            'publication_sections' => $article->publicationSections
                ...
                ->sortBy('sort_order')
                ->values(),
            'publication_files' => collect(app(PublishedArticleFileResolver::class)->resolvePublishedFiles($article))
                ->map(fn ($file) => [
                    'id' => $file->id,
    ...
    private function adminArticleSummaryPayload(Article $article, User $viewer): array
    {
        ...
        return [
            ...
            'status_projection' => app(LifecycleStatusProjector::class)->projection($article, $viewer),
    ...
    private function applyAdminArticleFilters($query, Request $request, bool $includeStatus = true): void
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('tracking_code', trim((string) $search))
                    ->orWhere('title', 'like', "%{$search}%")
                    ->orWhere('abstract', 'like', "%{$search}%")
                    ->orWhereHas('issue', function ($issueQuery) use ($search) {
                        $issueQuery->where('special_title', 'like', "%{$search}%")
                            ->orWhere('volume_number', 'like', "%{$search}%")
                            ->orWhere('issue_number', 'like', "%{$search}%")
                            ->orWhere('issue_month', 'like', "%{$search}%")
                            ->orWhere('issue_year', 'like', "%{$search}%");
                    })
        ...
        if ($request->filled('tracking_code')) {
            $query->where('tracking_code', trim((string) $request->query('tracking_code')));
        }
        ...
        if ($request->filled('issue')) {
            ...
            $query->whereHas('issue', function ($issueQuery) use ($issue) {
                $issueQuery->where('special_title', 'like', "%{$issue}%")
                    ->orWhere('volume_number', 'like', "%{$issue}%")
                    ->orWhere('issue_number', 'like', "%{$issue}%")
                    ->orWhere('issue_month', 'like', "%{$issue}%")
                    ->orWhere('issue_year', 'like', "%{$issue}%");
            });
        }
