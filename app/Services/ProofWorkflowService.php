...
{
    public function request(Article $article, User $actor, int $sourceFileId, ?string $dueAt, string $key): array
    {
        return app(ArticleLifecycleService::class)->command($article, $actor, 'request-proof', $key, ['article_file_id' => $sourceFileId, 'due_at' => $dueAt], 'proof.requested', 'author.final_review_requested', function (Article $locked) use ($sourceFileId, $dueAt, $key) {
            ...
            $round = ProofRound::create(['article_id' => $locked->id, 'article_version_id' => $set->article_version_id, 'accepted_file_set_id' => $set->id, 'production_assignment_id' => $assignment->id, 'round_number' => ((int) $locked->proofRounds()->max('round_number')) + 1, 'status' => 'awaiting_author', 'source_file_id' => $file->id, 'requested_at' => now(), 'due_at' => $dueAt, 'idempotency_key' => $key, 'active_marker' => 1]);
            $locked->update(['status' => ArticleStatus::PROOFREADING, 'author_final_review_requested_at' => now(), 'author_final_review_due_at' => $dueAt]);
    ...
    public function authorRespond(ProofRound $proof, User $actor, string $decision, ?string $comments, ?int $fileId, string $key): array
    {
        ...
        return app(ArticleLifecycleService::class)->command($proof->article, $actor, 'proof-author-response', $key, ['proof_round_id' => $proof->id, 'decision' => $decision, 'article_file_id' => $fileId], 'proof.author_'.$decision, $event, function (Article $locked) use ($proof, $actor, $decision, $comments, $fileId) {
            ...
            if ($decision === 'corrections' && ! trim((string) $comments) && ! $fileId) {
                app(ArticleLifecycleService::class)->conflict('Comments or an annotated proof are required when requesting corrections.');
            }
            ...
            $locked->update($decision === 'approve'
                ? ['status' => ArticleStatus::READY_FOR_PUBLICATION, 'author_final_approved_at' => now(), 'author_final_approved_by' => $actor->id]
    ...
    public function correct(ProofRound $proof, User $actor, int $fileId, ?string $notes, string $key): array
    {
        return app(ArticleLifecycleService::class)->command($proof->article, $actor, 'proof-correction', $key, ['proof_round_id' => $proof->id, 'article_file_id' => $fileId], 'proof.corrected', 'author.final_review_requested', function () use ($proof, $actor, $fileId, $notes) {
            ...
            $file = $assignment ? $round->article->files()->whereKey($fileId)->where('file_type', 'proof_file')->where('assignment_type', 'production_assignment')->where('assignment_id', $assignment->id)->where('scan_status', 'clean')->first() : null;
            if (! $assignment || ! $file) {
            ...
            $round->update(['status' => 'resent', 'corrected_file_id' => $file->id, 'production_notes' => $notes, 'requested_at' => now(), 'responded_at' => null]);
            $assignment->update(['status' => 'completed', 'completed_at' => now()]);

            return ['proof_round_id' => $round->id, 'status' => 'resent'];
        });
    }
}
